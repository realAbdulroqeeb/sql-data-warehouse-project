	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE WHEN UPPER(TRIM(cst_material_status)) = 'S' THEN 'Single'
				 ...
				 ELSE 'n/a'
			END AS cst_material_status, -- Normalize marital status values to a readable format
		...
		)t WHERE flag_last =  1 -- Select the most recent data 
		...
		SELECT 
			...
			CASE WHEN TRIM(cntry) IN ('USA','US','United States') THEN 'United States'
				 ...
				 ELSE TRIM(cntry)
			END cntry -- Normalize and hndle missing or blan country codes
		...
		SELECT 
		 ...
		 PRINT'Total Time Duration: ' + CAST(DATEDIFF(second,@start_batch,@end_batch) AS NVARCHAR) + ' seconds'
		 PRINT'=================================================='
